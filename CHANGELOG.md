...
- Connected revision-bound EFB classification to article creation, review,
  ...
  from source metadata/passages. Decisions retain registry/model provenance,
  human overrides and source invalidation. Independent rollout switches default
  off pending real-aviation evaluation.
- Added durable classification retries/reconciliation and queued bulk signed
  export with explicit revision/visual snapshots, article-specific preflight
  failures, reviewed PNG assets, and version metadata. Selected export now
  requires approved current revisions. Fixed YAML aliases that caused valid
  selected packages to fail frontmatter validation.
- Verified PostgreSQL version allocation, approval/selection and versioned edits,
  Redis retries, signed fixture export through Project EFB's validator, and
  metadata drift rejection. Added a reviewed-corpus precision report command.

- Added article revision version numbers, parent revision and change reason,
  visible version history, and exported article-version metadata. Migrations
  backfill existing history and serialize concurrent version allocation; the
  PostgreSQL migration/concurrency verification now passes.

- Added signed Project EFB `poc-cloud` export through the existing release exporter. Selected current approved articles now produce schema 2.1 native OKF, display, agent, and retrieval artifacts with consumer-registry aircraft and ATA/QRH placements, exact prototype authority labeling, Ed25519 integrity, structural consumer validation, and a non-secret acceptance report. Invalid, missing, mismatched, orphaned, or silently dropped placements fail the export; generic OKF export remains independent.
...
- Added the conservative agent rollout candidate without changing production authority. Chat now classifies evidence as strong, partial, weak, or none; records why raw RAG was invoked; checks the complete selected-bundle scope for approved OKF before falling back to raw discovery; and exposes these decisions in the trace panel. Bundle profiles gain a versioned `agent.boundedAdaptiveRetryEnabled` flag that defaults off. When explicitly enabled, one structured provider call may rephrase retrieval inside the same route and selected scope; route drift, scope requests, protected-identifier loss, equivalent or malformed output, provider failure, no improvement, and validation failure all restore the original deterministic result.
- Closed the bundle-deletion citation safety gap. The durable deletion worker now tombstones any historical assistant answer supported by the deleted bundle, including mixed-source answers, clears citations and retrieval trace details, removes attached knowledge gaps, preserves conversations that retain other sources, and records the tombstoned-answer count in the minimal deletion audit. Legacy citations are matched only through an exact single-bundle turn snapshot, preventing uncited or surviving-bundle answers from being removed by scope alone.
- Recorded the rebuilt Docker rollout baseline in `docs/debug/agent-rollout-route-coverage-2026-07-24.json`: all nine golden route questions, twelve Stage 7C release probes, and three Stage 7D multi-bundle probes passed with no citation regressions. The new probes verify mid-chat bundle addition/removal, visible exact-value conflicts, later-turn scope exclusion, and cross-workspace scope rejection. The separate bundle-deletion profile also confirmed deleted-bundle answer tombstoning, preserved read-only history, source preservation, and idempotent cleanup.
- Added approved-OKF citation drilldown from chat to source PDFs. OKF citations now keep the raw-RAG flow unchanged while opening a workspace-scoped approved-topic page, resolving the source document server-side from the active approved `TopicRecord.exportedFilePath`, and showing a Source document block with the full page range plus authenticated `Open PDF at source page` and document-record actions when available. Topic file and `returnTo` inputs are normalized and fail closed against traversal, cross-bundle/workspace lookups, inactive concepts, unavailable documents, and missing PDFs.
