...
export function NewChatPanel({ activeBundle }: { activeBundle: { id: string; name: string } | null }) {
  ...
  return (
    <div className="flex h-full min-h-[32rem] flex-col">
      <div className="flex shrink-0 items-center gap-3 border-b px-4 py-3 sm:px-6">
        ...
        <div><h1 className="text-sm font-semibold">New conversation</h1><p className="text-xs text-muted-foreground">{activeBundle?.name ?? "No knowledge source selected"}</p></div>
      </div>
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto px-4 py-10">
        <div className="w-full max-w-2xl text-center">
          ...
          <h2 className="mt-4 text-2xl font-semibold">Ask your approved knowledge</h2>
          <p className="mx-auto mt-2 max-w-lg text-sm text-muted-foreground">Answers are grounded in the active bundle. You can add other knowledge sources after the conversation starts.</p>
          {activeBundle ? <Badge className="mt-4" variant="outline">{activeBundle.name}</Badge> : null}
