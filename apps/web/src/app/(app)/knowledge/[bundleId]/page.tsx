      ...
      <section className="px-4 py-6 sm:px-6 lg:px-8">
        ...
        <details className="mb-4 border border-border bg-muted/10">
          ...
          <div className="space-y-4 border-t border-border p-4">
            ...
            {query.relationsDiscovered ? <div className="border border-emerald-400/30 bg-emerald-400/10 p-3 text-sm">Evaluated relation pairs; {query.relationsDiscovered} candidates met the deterministic threshold.</div> : null}
            <div className="space-y-2">{relationCandidates.filter((candidate) => candidate.status === "pending").map((candidate) => <div className="grid gap-3 border border-border p-3 text-xs lg:grid-cols-[1fr_auto] lg:items-center" key={candidate.id}><div><div className="font-medium">{candidate.sourceFile} <span className="text-muted-foreground">{candidate.relation}</span> {candidate.targetFile}</div><div className="mt-1 text-muted-foreground">{candidate.reason}</div></div><div className="flex gap-2"><form action={reviewRelationCandidateAction}><input name="candidateId" type="hidden" value={candidate.id} /><input name="decision" type="hidden" value="approve" /><PendingSubmitButton pendingLabel="Approving...">Approve</PendingSubmitButton></form><form action={reviewRelationCandidateAction}><input name="candidateId" type="hidden" value={candidate.id} /><input name="decision" type="hidden" value="reject" /><PendingSubmitButton pendingLabel="Rejecting...">Reject</PendingSubmitButton></form></div></div>)}</div>
          </div>
        </details>
        <details className="mb-4 border border-border bg-muted/10">
